...
KukaTreeNode* insert_node(const std::string &name, const std::string &abs_path, KukaTreeNode *prev_node) {
        ...
        new_node = new KukaTreeNode(name,get_ancestors(abs_path), parent_node);
...
KukaTreeNode* generate_tree(const std::vector<std::string> &frame_names, std::vector<std::string> &abs_paths) {
    ...
    KukaTreeNode* root = new KukaTreeNode("r", empty, nullptr);
    KukaTreeNode* prev_node = root;

    // create tree
    for(std::vector<std::string>::size_type i = 0; i != frame_names.size(); i++) {
        prev_node = insert_node(frame_names[i], abs_paths[i], prev_node);
    }
    return root;
}